...
def save_matrix_to_csv(matrix):
    with open("graph_matrix.csv", "w", newline='') as csv_file:
        writer = csv.writer(csv_file, delimiter=',')
        for row in matrix:
            writer.writerow(row)


def save_adjacency_list_to_csv(matrix):
    output_dict = {row: [] for row in range(len(matrix))}
    for row in range(len(matrix)):
        for column in range(len(matrix[row])):
            output_dict[row].extend([column for _ in range(matrix[row][column])])
    with open('graph_adjacency_list.csv', 'w') as f:
...
if __name__ == '__main__':
    if len(sys.argv) < 1 or sys.argv[1] == "new":
        ...
        # save_adjacency_list_to_csv(matrix)
    else:
